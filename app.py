...
def save_data(component_number, image_name="N/A"):
    ...
    # Try saving to Excel first (if openpyxl exists)
    try:
        ...
        # If Excel fails (e.g. creating file or missing library), try CSV
        # st.warning(f"Excel save failed ({e_xlsx}), trying CSV...")
        pass
    ...
    try:
        ...
        if os.path.exists(file_path_csv):
            ...
            if os.path.getsize(file_path_csv) > 0:
                df_new.to_csv(file_path_csv, mode='a', header=False, index=False)
            else:
                df_new.to_csv(file_path_csv, mode='w', header=True, index=False)
        else:
            df_new.to_csv(file_path_csv, mode='w', header=True, index=False)
        st.success(f"✅ Saved to CSV: {component_number}")
    except Exception as e_csv:
        st.error(f"❌ Error saving to CSV: {e_csv}")

st.set_page_config(page_title="Scanner Komponen", page_icon="📷")

st.title("📷 Admin Scanner Komponen")
st.markdown("Scan nomor komponen dan simpan otomatis ke **Excel/CSV**.")
...
with col2:
    ...
    if img_file_buffer is not None:
        # Load image with PIL to be safer/lighter than cv2 if not needed
        ...
        with st.status('Sedang memproses OCR (Cloud/Local)...', expanded=True) as status:
            try:
                ...
                if detected_text:
                    status.write(f"Terdeteksi: {detected_text}")
                    
                    status.write("Menyimpan ke Excel/CSV...")
                    save_data(detected_text, "Camera Input")
                    status.update(label="Selesai!", state="complete", expanded=True)
                    
                    st.toast(f"Tersimpan: {detected_text}")
                else:
